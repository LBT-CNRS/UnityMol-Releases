...
public class UnityMolHBallMeshManager : UnityMolGenericRepresentationManager {
		...
		if (atomRep.atomToId.TryGetValue(a, out idAtom)) {
			if (idAtom != -1) {
				if (coordAtomTexture.TryGetValue(idAtom, out keyValP)) {
					...
					texturesAtoms[keyValP.Key].SetPixel(keyValP.Value, 9, Vector4.one);
		...
		if (atomRep.atomToId.TryGetValue(a, out idAtom)) {
			if (idAtom != -1) {
				if (coordAtomTexture.TryGetValue(idAtom, out keyValP)) {
					...
					texturesAtoms[keyValP.Key].SetPixel(keyValP.Value, 9, Vector4.zero);
	...
	public void updateAOInfo(List<Vector2> atlasinfo) {
		...
		for (int i = 0; i < atlasinfo.Count; i++) {
			if (coordAtomTexture.TryGetValue(i, out keyValP)) {
				tmp.x = atlasinfo[i].x;
				tmp.y = atlasinfo[i].y;
				texturesAtoms[keyValP.Key].SetPixel(keyValP.Value, 10, tmp);
	...
	public void cleanAO() {
		...
		for (int i = 0; i < meshesGO.Count; i++) {
			meshesGO[i].GetComponent<Renderer>().sharedMaterial.SetFloat("_AOStrength", 0.0f);
		}
	}
