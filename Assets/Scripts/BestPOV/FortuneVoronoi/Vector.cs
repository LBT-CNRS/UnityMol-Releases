...
public class Vector : IEnumerable, IComparable
{
	...
	public Vector(string S)
	{
		...
		data = new float[P.Length];
		int i;
		for (i = 0; i < data.Length; i++)
		{
			data[i] = float.Parse(P[i]);
		}
	}
	/// <summary>
	...
	public float this [int i]
	{
		...
		set
		{
			data[i] = (float)Math.Round(value, Precision);
		}
	}
	...
	public void Randomize(float Min, float Max)
	{
		...
		for (i = 0; i < data.Length; i++)
		{
			this[i] = Min + (Max - Min) * (float)MathTools.R.NextDouble();
		}
	}
	/// <summary>
	/// Reset all elements with ransom values from the given range
	...
	public void Randomize(Vector[] MinMax)
	{
		...
		for (i = 0; i < data.Length; i++)
		{
			this[i] = MinMax[0][i] + (MinMax[1][i] - MinMax[0][i]) * (float)MathTools.R.NextDouble();
		}
	}
	/// <summary>
