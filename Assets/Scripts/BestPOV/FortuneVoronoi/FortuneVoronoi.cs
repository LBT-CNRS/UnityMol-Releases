...
//			/////////////////////
	...
	internal class VEdgeNode : VNode
	{
		...
		{
			if(!Flipped)
				return (float)Math.Round(x-Fortune.ParabolicCut(Edge.LeftData[0], Edge.LeftData[1], Edge.RightData[0], Edge.RightData[1], ys),10);
			return (float)Math.Round(x-Fortune.ParabolicCut(Edge.RightData[0], Edge.RightData[1], Edge.LeftData[0], Edge.LeftData[1], ys),10);
		}
	}
	...
	internal class VCircleEvent : VEvent
	{
		...
		public override float Y
		{
			get
			{
				return (float)Math.Round(Center[1]+MathTools.Dist(NodeN.DataPoint[0],NodeN.DataPoint[1],Center[0],Center[1]),10);
			}
		}
	...
	{
		public static readonly Vector VVInfinite = new Vector(float.PositiveInfinity, float.PositiveInfinity);
		public static readonly Vector VVUnkown = new Vector(float.NaN, float.NaN);
...
//			}
			...
			if(Math.Abs(a1-a2)<1e-10)
				return (x1+x2)/2;
			float xs1 = 0.5f/(2*a1-2*a2)*(4*a1*x1-4*a2*x2+2*Mathf.Sqrt(-8*a1*x1*a2*x2-2*a1*y1+2*a1*y2+4*a1*a2*x2*x2+2*a2*y1+4*a2*a1*x1*x1-2*a2*y2));
			float xs2 = 0.5f/(2*a1-2*a2)*(4*a1*x1-4*a2*x2-2*Mathf.Sqrt(-8*a1*x1*a2*x2-2*a1*y1+2*a1*y2+4*a1*a2*x2*x2+2*a2*y1+4*a2*a1*x1*x1-2*a2*y2));
